...
`;var v=(o=>(o.New="new",o.Done="done",o.Warn="warn",o.Error="error",o))(v||{});function ke(o,e){const i=o.userData.sourceDetails.find(n=>n.id===e.id)?.selectedTags??[];if(i.length===0)return{iconState:"new",collidingTagIds:[]};const s=e.colisionMode??M.None;if(s===M.None)return{iconState:"done",collidingTagIds:[]};const r=new Set;for(const n of o.userData.sourceDetails){if(n.id===e.id)continue;const l=o.sourceDetails.find(a=>a.id===n.id);if(l&&l.selectionId===e.selectionId)for(const a of n.selectedTags)i.includes(a)&&r.add(a)}return r.size===0?{iconState:"done",collidingTagIds:[]}:{iconState:s===M.Error?"error":"warn",collidingTagIds:Array.from(r)}}function rt(o,e){return ke(o,e).iconState}function nt(o,e){const t=o.userData.sourceDetails.find(s=>s.id===e.id)?.selectedTags??[],i=o.selections.find(s=>s.id===e.selectionId);return t.map(s=>i?.tags.find(r=>r.id===s)?.value??s)}function at(o,e){if(e.dependencyLinks.length===0)return[];const t=[];for(const i of e.dependencyLinks)for(const s of(o.dependencyData??[]).filter(r=>r.moduleType==="swt")){const r=s.userData.sourceDetails.find(n=>n.id===i);if(r)for(const n of r.selectedTags)n.tagValue&&t.push(n.tagValue)}return t}function lt(o,e,t){return o.sourceDetails.map(i=>ct({uiModel:o,item:i,onSelect:e,onKeydown:t}))}function ct(o){const{uiModel:e,item:t,onSelect:i,onKeydown:s}=o,r=Number(t.position?.x??0),n=Number(t.position?.y??0),l=60,a=rt(e,t),c={[v.New]:tt,[v.Done]:st,[v.Warn]:it,[v.Error]:ot};return b`
  ...
  `}function dt(o){return o.sourceDetails.flatMap(e=>{const t=nt(o,e),i=at(o,e);if(t.length===0&&i.length===0)return[];const s=Number(e.position?.x??0),r=Number(e.position?.y??0);return[b`
        <foreignObject
          x=${s}
          y=${r}
          width="1"
          height="1"
          overflow="visible"
          style="overflow: visible; width: 104px; height: 54px; transform: translate(1px, -27px); pointer-events: none;"
        >
          <div class="swt__tag-box">
            <div class="swt__tag-top">${t[0]||""}</div>
            <div class="swt__tag-bottom">${i[0]||""}</div>
          </div>
        </foreignObject>
      `]})}function ht(o){const{configuration:e,uiModel:t,onSelect:i,onKeydown:s,onImgLoad:r,onModalClose:n,onTagSelect:l}=o,a=t.imgWidth>0?t.imgWidth:0,c=t.imgHeight>0?t.imgHeight:0;return _`
    <div class="swt">
      ${e.source?_`<img
